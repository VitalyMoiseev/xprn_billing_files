...
if ($olt_gpon){
    foreach ($GPON_Vendors as $search => $replace) {
        $mac_show = str_replace($search, $replace, $mac);
    }
}else{
    $mac_show = $mac;
}
echo $mac_show;
echo '</span>';
echo " <a href=\"$usInvFindStr$mac_show\" target=\"_blank\"><sup>[склад]</sup></a>";
echo '</td></tr><tr>';
...
echo "</td><td class=\"$tdclass\"><b>$onu_s</b>";
if($pwr != 0){
    echo " Db";
}
...
function manage_ONU_OLT(olt_id, onu_key){
    ...
    var url1 = "/scripts/manage_ONU_OLT.php?olt_id=" + olt_id + "&onu_key=" + onu_key + "&onu_name=" + onu_name + "&thost=" + thost + "&tlog=" + tlog + "&tpas=" + tpas + "&host=" + host + "&comrw=" + comrw + "&type=" + type;
    $('#onu_data_field').load(url1);
}
function hide_me(){
